...
# kind поступают при вызове метода __init__. При этом значение kindне является обязательным и
...
# brand, max_speed , kind родительскому классу Transport , при этом атрибуту kindпередайте значение "Car";
#
...
# gasoline_residue на переданное значение и затем вывести фразу 'Объем топлива увеличен на <value> л и '
# 'составляет <gasoline_residue> л'. Если в значение value подается не целое число, вывести 'Ошибка заправки автомобиля'.
#
#
# В классе Boatдолжны быть реализованы:
#
# метод __init__, принимающий три значения brand, max_speed, owners_name. Их нужно сохранить в соответствующие атрибуты.
# При этом внутри инициализации нужно делегировать создание атрибутов brand, max_speed , kind родительскому
# классу Transport , в атрибут kind при этом передайте значение "Boat";
#
...
class Car(Transport):
    ...
    def __init__(self, brand, max_speed, mileage, gasoline):
        super().__init__(brand, max_speed, 'Car')
        self.mileage = mileage
        self.__gasoline_residue = gasoline
