...
#
# определить метод __call__. Он должен принимать произвольное количество аргументов и среди этих аргументов
# находить числа и их суммировать. Все остальные типы данных необходимо пропускать.
...
class Addition:
    ...
    def __call__(self, *args):
        data = list(filter(lambda x: type(x) in (int, float), args))
        return f'Сумма переданных значений = {sum(data)}'
